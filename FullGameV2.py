...
class Game:
    def __init__(self, strPath):
        ...
        self.winner = []
        self.loadFile(strPath)
        self.createPlayer(self.game)
        self.findWinner()

    # Old-Ver.1
    # def loadFile(self, strFilePath):            #03 - v1  
    #     with open(strFilePath) as file:            
    #         self.game = file.readlines()[2:-2]  #readlines() read into a List, skip first and last two lines
    #         # print(f'game: ', self.game)       #['3\n', '0 Kh 4d 3c\n', '1 Jd 5c 7s\n', '2 9s 3h 2d\n']

    # def createPlayer(self, lsGame):     
    #     numOfPlayer = int(lsGame[0].rstrip('\n'))
    #     # print(f'numOfPlayer: ', numOfPlayer)

    #     for i in range(1, numOfPlayer+1):
    #         playersCard = lsGame[i].rstrip('\n').split()  #['0', 'Kh', '4d', '3c']
